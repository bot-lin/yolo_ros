...
- **use_stream_source**: whether to run direct URL stream inference inside yolo_node (default: False)
- **stream_source**: direct URL stream source, for example RTSP/HTTP MJPEG (default: empty)
- **stream_frame_id**: frame ID used in detection header when direct stream mode is enabled (default: camera_link)
